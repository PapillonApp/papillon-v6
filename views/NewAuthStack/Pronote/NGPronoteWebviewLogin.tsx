...
import { getPronoteInstanceInformation, defaultPawnoteFetcher, type PronoteInstanceInformation } from 'pawnote';
...
const NGPronoteWebviewLogin = ({ route, navigation }: {
  ...
  (function() {
      try {
          var lJetonCas = "";
          var lJson = JSON.parse(document.body.innerText);

          window.ReactNativeWebView.postMessage('json :' + JSON.stringify(lJson));
          ...
          window.ReactNativeWebView.postMessage('e :' + e);
          return false;
      }
  })();
  `;
  ...
  const INJECT_LOGIN = `
    (function(){
      window.ReactNativeWebView.postMessage(window && window.loginState ? JSON.stringify(window.loginState) : '(vide)');
  ...
  const onMessage = (event: any) => {
    ...
    console.log('[NGPWVL] Received message:', data);

    if (data.trim() == 'nextStep:1') {
      console.log('[NGPWVL] Next step...');
      webViewRef.current.injectJavaScript(`
        window.location.href = "${instanceURL}?fd=1";
      `);
    }
  };
